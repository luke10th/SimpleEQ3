...
void SimpleEQ3AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    spec.maximumBlockSize = samplesPerBlock;  // max number of samples that will processed at once
    spec.numChannels = 1;
...
void SimpleEQ3AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    auto leftBlock = block.getSingleChannelBlock(0);  // setting up audio blocks representing each mono channel
